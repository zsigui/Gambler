...
{
    public partial class X469DialogConfirm : Form
    {
        private static X469DialogConfirm sInstance;

        public static X469DialogConfirm newInstance()
        {
            if (sInstance == null)
            {
                sInstance = new X469DialogConfirm();
            }
            else
            {
                sInstance.Focus();
        ...
        public void Update(X469OddItem oddData, string oddInfo, string ltype, string bet, bool autoOpt, string odd, bool isAuto)
        {
            ...
            {
                if (account.Type != AcccountType.XPJ469)
                    continue;
        ...
        private void DoRequest(bool isAuto)
        {
            ...
            ThreadUtil.RunOnThread(() =>
            {
                ...
                for (int i = 0; i < _accounts.Count; i++)
                {
                    ...
                    tmpAccount.GetClient<X469Client>().DoBet(_requestData,
                            (ret) =>
                            {
                                UpdateDGVCellOnThread(i, "√");
                            },
