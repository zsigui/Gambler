...
public class ImagePreProcess6 {
	...
	public static void getSvmdata() throws Exception {
		...
		File dataFile = new File("train6\\data.txt");
	...
	public static void scaleTraindata() throws Exception {
		...
		for (File file : files) {
			...
			ImageIO.write(imgdest, "JPG", new File("train6\\" + file.getName()));
			fs.write((file.getName().charAt(0) + " ").getBytes());
			int index = 1;
			for (int x = 0; x < imgdest.getWidth(); ++x) {
				for (int y = 0; y < imgdest.getHeight(); ++y) {
	...
	public static void main(String[] args) throws Exception {
		...
		svm_predict.main(new String[] {
				new File("train6\\data.txt").getAbsolutePath(),
				new File("train6\\data.txt.model").getAbsolutePath(),
				"train5\\output.txt" });
	}
}
